...
1. 构造原则（引用原文）  
   ...
   Принцип построения (цитата)  
   ‑ «Чем выше уровень в иерархии, тем выше скорость… тем дороже, поэтому объём становится меньше»  
   ‑ Регистры → кэш-уровни → оперативка → жёсткий диск → ленточный робот  

2. 生活类比（引用原文）  
   ‑ 工作台=寄存器，抽屉=缓存，工具柜=主存，仓库=磁盘，外厂订单=磁带  
   ‑ 目的：让“手边”永远放着下一步最可能用的指令和数据  

   Бытовая аналогия (цитата)  
   ‑ «Необходимое — на столе (регистры), часто используемое — в ящиках (кэш), основной набор — в шкафу (ОЗУ), редкое — на складе (диск)»  
...
--------------------------------------------------
1. 定义与典型场景（引用原文）  
   ‑ “Идеальный пример … — это цикл с телом из одного оператора и большим числом итераций”  
   ‑ 一旦进入循环，CPU 将无数次取同一条指令 → 适合锁进缓存或预取  

   Определение и пример (цитата)  
   ‑ «Идеальный пример фрагмента с высокой локальностью вычислений — цикл … с большим числом итераций»  
   ‑ Повторная выборка тех же команд → выгодно разместить в быстрой памяти  

2. 破坏因素  
   ‑ 频繁过程调用、条件跳转、间接分支 → 指令流“跳远”，局部性下降  

   Факторы-разрушители  
   ‑ Частые вызовы подпрограмм, условные переходы → поток команд «уходит далеко»  

--------------------------------------------------
三、数据局部性 —— 时间+空间上的“反复访问同一数据”  
III. Локальность данных — временное и пространственное повторение  
--------------------------------------------------
1. 时间局部性（引用原文）  
   ‑ “Всё то, что используется часто, лучше хранить на регистрах”  
   ‑ 循环计数器、累加变量被反复读写 → 留在寄存器最划算  

   Временная локальность (цитата)  
   ‑ «Всё, что используется часто, лучше хранить на регистрах»  
   ‑ Повторные чтения/записи одной и той же переменной → держим в регистрах  

2. 空间局部性（引用原文）  
   ‑ “Следующий … объект … расположен в оперативной памяти ‘недалеко’ от предыдущего”  
   ‑ 顺序遍历数组、结构体字段相邻 → 一次缓存行加载可复用多次  

   Пространственная локальность (цитата)  
   ‑ «Следующий объект находится “недалеко” от предыдущего»  
   ‑ Последовательный обход массива → одна строка кэша обслуживает множество обращений
