...
**Компьютеры Cm*, BBN Butterfly**
--------------------------------------------------
一、 Суперкомпьютер BBN Butterfly (BBN Butterfly 超级计算机)
I. Суперкомпьютер BBN Butterfly
--------------------------------------------------
- **Архитектура**: Классический представитель ранних систем с **распределённой общей памятью**.
    - **架构**: 早期**分布式共享内存**系统的经典代表。
- **Ключевая черта**: Процессорные модули на коммерческих микропроцессорах соединялись через высокоскоростную **сеть «бабочка» (butterfly/omega)**.
    - **关键特征**: 基于商用微处理器的处理器模块通过高速**蝶形网络**互联。
- **Значение**: Успешная коммерческая система, показавшая практичность масштабируемых мультипроцессоров с общей памятью.
    - **意义**: 成功的商业系统，证明了可扩展共享内存多处理器的实用性。

--------------------------------------------------
二、 Кластерная система Cm* (集群系统 Cm*)
II. Кластерная система Cm*
--------------------------------------------------
- **Архитектура**: Иерархическая исследовательская система, яркий пример **архитектуры NUMA**.
    - **架构**: 层次化研究性系统，是**NUMA架构**的鲜明范例。
- **Ключевая черта**: Состояла из кластеров процессоров (по 14 шт.), доступ к памяти внутри кластера был быстрым, а между кластерами — медленным.
    - **关键特征**: 由处理器集群（每集群14个）组成，集群内内存访问快，集群间访问慢。
- **Значение**: Важный исследовательский проект, углубивший понимание проблем когерентности памяти и масштабируемости.
    - **意义**: 重要的研究项目，深化了对内存一致性和可扩展性问题的理解。

--------------------------------------------------
**Сравнение и контекст (对比与背景)**
--------------------------------------------------
Оба компьютера были **пионерами в области распределённой общей памяти (DSM/NUMA)** в 1970-80-х годах. **Butterfly** — практическая коммерческая реализация. **Cm*** — глубокое академическое исследование иерархической организации. Их идеи повлияли на современные кластеры и многопроцессорные системы.
这两台计算机都是1970-80年代**分布式共享内存技术的先驱**。**Butterfly**是实用的商业实现，而**Cm***是关于层次化组织的深入学术研究。它们的理念影响了现代集群和多处理器系统。
...
--------------------------------------------------
一、产品定位与体系结构  
I. Позиционирование и архитектура  
--------------------------------------------------
1. “矢量超算卡”——把传统 SX 矢量机浓缩成一张 PCIe  
   ‑ 1.6 GHz × 8 矢量核，每周期 32 条双精度乘加  
   ‑ 理论峰值 2.45 TFLOPS（FP64）/ 4.9 TFLOPS（FP32）  
   ‑ 片上 48 GB HBM2，带宽 1.53 TB/s（≈同时代 GPU 的 2×）  

   «Векторный суперкомпьютер на карте»  
   ‑ 8 векторных ядер, 1,6 ГГц, 32 FMA/такт  
   ‑ Пиковая производительность 2,45 Тфлопс (FP64)  
   ‑ 48 ГБ HBM2, пропускная способность 1,53 ТБ/с  

2. 宿主–加速分离式内存模型  
   ‑ 宿主：标准 x86_64 Linux，负责 OS、I/O、调度  
   ‑ 加速卡：运行完整轻量内核 “VEOS”，进程直接分配 48 GB 本地内存  
   ‑ 通信：PCIe 3.0 ×16 + 低延迟驱动，对用户透明  

   Модель «хост + ускоритель»  
   ‑ Хост: обычный сервер под Linux  
   ‑ Карта: легковесное ядро VEOS, 48 ГБ локальной памяти  
   ‑ Данные пересылаются по PCIe, но для программиста выглядит как shared-memory  

--------------------------------------------------
二、编程与软件生态  
II. Программирование и экосистема  
--------------------------------------------------
1. 语言与编译器  
   ‑ Fortran/C/C++ 无需改写，只需重编译  
   ‑ 自动矢量化 + 自动并行化（-fvectorize / -fopenmp）  
   ‑ 向量化报告帮助定位未矢量化的循环  

   Языки и компилятор  
   ‑ Исходный код остаётся без изменений  
   ‑ Автоматическая векторизация и OpenMP  

2. 数学库与框架  
   ‑ 高度优化的 BLAS、LAPACK、ScaLAPACK、FFTW  
   ‑ 支持 HPC-AI 混合：Python + NumPy/SciPy 通过 pip-wheel 直接调用 VE  

   Библиотеки  
   ‑ Векторизованные линейная алгебра, FFT  
   ‑ Готовые Python-wheel для NumPy/SciPy  

3. 调试与性能分析  
   ‑ NEC “ftrace” 可视化矢量长度、带宽利用率  
   ‑ gdb 扩展支持单核/多核调试  

   Отладка  
   ‑ Инструменты трассировки векторных операций  

--------------------------------------------------
三、性能特征与对比  
III. Производительность и сравнение  
--------------------------------------------------
1. 内存带宽极限型应用  
   ‑ STREAM-Triad 实测 1.3 TB/s → 比同代 CPU 高 10×，与 GPU 相当但无需显式传输  
   ‑ 气象、CFD、量子化学等“带宽饥饿”代码可获 5–10× 加速  

   Зависимые от полосы пропускания  
   ‑ STREAM 1,3 ТБ/с — в 10 раз быстрее CPU  

2. 延迟与扩展  
   ‑ 单卡 300 W，2U 服务器可插 4–8 张 → 节点 10–20 TFLOPS  
   ‑ 节点间仍用 InfiniBand 拼集群，弱扩展接近线性  

   Масштабируемость  
   ‑ 4–8 карт в 2U-сервере, далее стандартный IB-кластер  
...
--------------------------------------------------
1. 数值天气预报  
   ‑ 德国气象局 ICON 模式：单节点 VE 替换 4×CPU 节点，能耗 ↓40 %  

   Прогноз погоды  
   ‑ ICON от DWD: замена 4 × CPU на 1 × VE, экономия энергии 40 %  

2. 工业 CAE  
   ‑ 汽车/航空 CFD：高矢量长度 → 残差收敛更快，设计周期缩短  

   Промышленное моделирование  
   ‑ Аэродинамика: ускорение сходимости  

3. AI + HPC 混合  
   ‑ 特征工程阶段需高内存带宽，VE 在 48 GB 内“全速”处理百亿级样本  

   Гибридные HPC-AI задачи  
   ‑ Инженерные признаки для ML  

--------------------------------------------------
五、小结：NEC SX-Aurora TSUBASA 带来的启示  
V. Вывод  
--------------------------------------------------
- 矢量架构并未消亡，只是“瘦身”成 PCIe 卡  
- 用极端内存带宽而非众核数量赢得应用  
- 对程序员友好：重编译即可，无需重写算法  
- 为“带宽饥饿”型科学计算与 AI 预处理提供了 CPU-GPU 之外的第三条路线
...
一、向量/SIMD 扩展的“代际路线图”  
I. Поколения векторных расширений  
--------------------------------------------------
1. 初代 64-bit：MMX (1996)  
   ‑ 8 个 64 mm-регистров，仅整数，复用 x87 浮点栈  
   MMX — 8 64-битных регистров mm0–mm7, только целые, повторно используют x87-стек  

2. 128-bit 时代：SSE(1-4) / AltiVec / ARM NEON  
   ‑ 寄存器 128 bit，16/32 个 XMM/VR/V  
   SSE/AltiVec/NEON — 128-битные регистры, 4×fp32 или 16×int8, задают «базовый набор»  

3. 256-bit 时代：AVX/AVX2 (2011/13)  
   ‑ YMM 寄存器 256 bit，3-операндное кодирование  
   AVX/AVX2 — 256-битные YMM, 8×fp32, FMA, gather, 3-операндный синтаксис  

4. 512-bit 时代：AVX-512 (2016+)  
   ‑ ZMM 512 bit × 32 + 8 个 маск-регистров k0–k7  
   AVX-512 — 512-битные ZMM и маски k0–k7, 16×fp32, подмножества F/VL/BW/VNNI  

5. 可变长时代：ARM SVE/SVE2 (2016/2021)  
   ‑ 向量长度由实现决定 128–2048 bit，二进制兼容  
   SVE — вектор «неизвестной» длины 128-2048 бит, предикаты, gather/scatter, бинарно-совместимый  

--------------------------------------------------
二、横向对比：同代 SIMD “谁能算多少”  
II. Сравнение пропускной способности  
-------------------------------------------------
| 扩展 | 宽 | 并行 fp32 | 并行 int8 | 特殊能力 | 典型芯片 |
|----|----|-----------|-----------|-----------|----------|
| SSE | 128 | 4 | 16 | 标量+向量 | Core 2, A53 |
| AVX2 | 256 | 8 | 32 | FMA, 3-op | Haswell, Ryzen |
| AVX-512 | 512 | 16 | 64 | маска, VNNI | Xeon, Ice-Lake-SP |
| SVE-256 | 256 | 8 | 32 | 可变长, 谓词 | Fujitsu A64FX |
| SVE-512 | 512 | 16 | 64 | 同上 | 未来 ARM 核 |

--------------------------------------------------
三、核心机制速览  
III. Ключевые механизмы  
--------------------------------------------------
1. 寄存器-文件  
   ‑ x86：MMX(64) → XMM(128) → YMM(256) → ZMM(512) 向上叠加  
   Регистровые файлы: x86 — наслаиваются; Power — 32 VR; ARM — 32 NEON → 32 SVE скалируемых  

2. 谓词/掩码 (Predicate/Mask)  
   ‑ AVX-512 k-регистры：每位对应通道，使能/禁用计算与回写  
   Предикаты: AVX-512 (k0–k7) и SVE (p0–p15) позволяют маскировать отдельные элементы  

3. Gather / Scatter  
   ‑ 用索引数组一次性加载/写入不连续地址，解决稀疏算法痛点  
   Gather/Scatter — загрузка/запись по индексам, решают «разреженные» задачи  

4. FMA (Fused Multiply-Add)  
   ‑ 单指令完成 ±a×b+c，提升矩阵乘、卷积、DFT 的乘加密度与精度  
   FMA — объединённое умножение-сложение за один такт, повышает точность и throughput  

--------------------------------------------------
四、编程与调优实战  
IV. Программирование и тонкая настройка  
-------------------------------------------------
1. 自动向量化  
   ‑ 编译器：-O3 -mavx2 -mfma / -msve -march=armv8.2-a+sve  
   Автоматическое векторизирование: ключи GCC/Clang -mavx2, -msve  

2. Intrinsics (内联函数)  
   ‑ x86：`_mm256_fmadd_ps(a,b,c)` → 一条 vfmadd132ps  
   ‑ ARM：`svmla_x(pred, acc, op1, op2)` → SVE 乘加带谓词  
   Интринсики: `_mm256_*` для AVX, `sv*_x()` для SVE  

3. 性能陷阱  
   ‑ AVX-512 高功耗 → 核心降频 100–300 MHz，需权衡“宽 vs. 频”  
   ‑ 跨核迁移：保存 ZMM 上下文增大任务切换开销  
   Ловушки: AVX-512 снижает частоту; контекст 512 бит тяжёлый для переключений  

--------------------------------------------------
五、典型加速效果  
V. Типовой прирост  
-------------------------------------------------
- 多媒体：H.264→H.265 转码，AVX2 比 SSE 快 ≈1.8×  
  Мультимедиа: транскодинг H.265 +AVX2 ≈ ×1.8 к SSE  

- 科学计算：双精度 GEMM，AVX-512 + FMA 比 AVX2 再快 ≈1.9×  
  Научные расчёты: DGEMM +AVX-512 ≈ ×1.9 к AVX2  

- AI 推理：INT8 卷积，VNNI 单指令 128 次乘加，相对 SSE 提升 4–6×  
  ИИ-инференс: INT8-conv +VNNI даёт ×4–6 к SSE  

- 超算：Fugaku A64FX (SVE-512) 在 HPL 效率 94 %，显示范例 scalable 向量长度优势  
  Суперкомпьютеры: A64FX (SVE-512) показывает 94 % эффективности в HPL  
...
-------------------------------------------------
MMX  → 64b 整数，已死  
MMX — 64-битные целые, устарело  

SSE  → 128b 基线，到处都有  
SSE — 128-бит «база», есть везде  

AVX2 → 256b 桌面/服务器主流  
AVX2 — 256-бит мейнстрим ПК/сервер  

AVX-512 → 512b 服务器，掩码+降频  
AVX-512 — 512-бит, маски, но понижает частоту  

AltiVec/VSX → Power 阵营 128b  
AltiVec/VSX — 128-бит мир Power  

NEON → ARM 128b 固定  
NEON — фиксированные 128 бит в ARM  

SVE → ARM 可变长，未来 HPC/AI  
SVE — масштабируемая длина, будущее HPC/ИИ
...
MPI: параллельная программа, сообщение, понятия групп и коммуникаторов.  
MPI：并行程序、消息、进程组与通信子概念。
MPI：для компьютеров с распределенной памятью
Сообщение – массив однотипных данных, расположенных в последовательных ячейках памяти.
消息——由同类型数据构成的数组，这些数据存储在连续的内存单元中
Вместе с сообщением по сети также посылается некий набор атрибутов: номер
процессаотправителя, номер процесса-получателя, идентификатор сообщения, коммуниктор.
与网络消息一同发送的还包含一组属性：发送方进程号、接收方进程号、消息
标识符、通信者。

Процессы также объединяются в группы – упорядоченные множества процессов. 
От понятия группы нужно отличать понятие коммуникатора – контекста обмена
группы. Это глобальный объект. Он объединяет различные процессы.
流程还被整合为组——即有序的流程集合。
需要区分“群组”概念与通信器——即群组交换的上下文环境。通信器是全局
性对象，它整合了各类进程。

MPI_COMM_WORLD – коммуникатор для всех процессов приложения. 用于所有应用程序进程的通信器。
MPI_COMM_SELF – коммуникатор, включающий только текущий процесс. 仅包含当前进程的通信器。
MPI_COMM_NULL – коммуникатор, не содержащий ни одного процесса .未包含任何进程的通信器。

## 35. MPI: синхронное взаимодействие процессов, виды операторов Send (Bsend, Ssend, Rsend). Тупиковые ситуации.
Функция MPI_Send является блокирующей. То есть текущий процесс приостанавливается до того момента, пока сообщение из буфера buf куда-то не ушло.
阻塞（同步）发送：调用后进程挂起，直到消息至少已从用户缓冲区拷出（MPI_Send），或直到对方已开始接收（MPI_Ssend）才返回。
...
阻塞（同步）接收：调用后进程挂起，直到所需消息完全到达并拷入用户缓冲区才返回（MPI_Recv）

MPI_Send
Стандартная блокирующая отправка. Возвращаемое значение функции гарантирует только то, что сообщение покинуло буфер отправителя. Не гарантируется, что сообщение дошло до получателя или что оно уже принято.
标准阻塞发送。函数返回只保证“消息已送出缓冲区”，至于是否到达接收方、是否被接收，不保证。
MPI_Bsend (Buffered-send)
Блокирующая отправка с использованием пользовательского буфера.
Перед использованием необходимо вызвать MPI_Buffer_attach, чтобы зарегистрировать буфер.
Функция возвращает управление сразу после копирования данных в буфер, не дожидаясь начала приёма.
带用户缓冲的阻塞发送。
必须事先调用 MPI_Buffer_attach 把一块内存注册为系统缓冲；
返回条件：数据被拷贝进该缓冲即可，无需等待接收动作。
MPI_Ssend (Synchronous-send)
Настоящая синхронная отправка: функция возвращает управление только тогда, когда получатель уже начал выполнение соответствующего 
MPI_Recv.
Таким образом, гарантируется, что приём начался — это самая строгая форма синхронизации.
真正“同步”发送：只有当对方已经开始执行匹配的 MPI_Recv 后，本函数才返回。
因此能保证“接收端已经开始收”，是最严格的同步语义。
MPI_Rsend (Ready-send)
Готовая отправка — самая быстрая, но требует, чтобы на момент вызова получатель уже начал приём (MPI_Recv уже вызван).
Если это условие не выполнено, поведение не определено (возможен сбой). Используется только тогда, когда программист точно уверен в готовности получателя.
“就绪”发送，速度最快，但前提条件是：调用它时，对方匹配的 MPI_Recv 必须已经启动；否则行为未定义（可能崩溃）。
一般只在程序员能 100% 保证接收端已准备好时才用。

Ситуации взаимной блокировки (тупиковые ситуации)
Классический пример тупика:
Процесс 0: MPI_Send(→1, …) → MPI_Recv(←1, …)
Процесс 1: MPI_Send(→0, …) → MPI_Recv(←0, …)
Оба процесса блокируются на Send, ожидая освобождения буфера, но не могут дойти до Recv — возникает взаимная блокировка.
经典死锁例子
进程 0：MPI_Send(→1, …) → MPI_Recv(←1, …)
进程 1：MPI_Send(→0, …) → MPI_Recv(←0, …)
结果：双方都在阻塞 Send 里等缓冲区，又都无法到达 Recv，永远互相等待。
Ложное решение — "перевёрнутый порядок":
Процесс 0: MPI_Recv(←1, …) → MPI_Send(→1, …)
Процесс 1: MPI_Recv(←0, …) → MPI_Send(→0, …)
Если внутренних буферов достаточно, Send может завершиться немедленно, и программа пройдёт.
Но при нехватке буферов (например, при больших сообщениях или многократном запуске) тупик всё равно возможен — этот способ ненадёжен.
“偶然成功”的伪解
把顺序颠倒：
进程 0：MPI_Recv(←1, …) → MPI_Send(→1, …)
进程 1：MPI_Recv(←0, …) → MPI_Send(→0, …)
如果系统内部缓冲足够，Send 会立即把数据拷走，程序能跑通；
但一旦缓冲不足（大消息或多次运行），就会再次死锁——所以这种写法依旧不安全。
Три надёжных способа избежать тупика:
Перекрёстный приём-передача:
Процесс 0: Send(→1) → Recv(←1)
Процесс 1: Recv(←0) → Send(→0)
Использовать неблокирующие операции:
MPI_Isend / MPI_Irecv + MPI_Wait / MPI_Test
Использовать комбинированные операции:
MPI_Sendrecv или MPI_Sendrecv_replace — одним вызовом выполняют и отправку, и приём, гарантируя отсутствие тупиков.
资料给出的 3 种正规解法
① 交叉收发：
进程 0：Send(→1) → Recv(←1)
进程 1：Recv(←0) → Send(→0)
② 使用非阻塞操作（Isend/Irecv + Wait/Test）
③ 直接用 MPI_Sendrecv 或 MPI_Sendrecv_replace——一次调用完成“收发对”，系统内部保证无死锁。

36 MPI: асинхронное взаимодействие процессов. 
避免死锁（deadlock）：两个进程互相等待对方先收/发就会永远卡死。
提高利用率：在消息传输的同时可以继续计算，不必空等。

int MPI_Isend … Неблокирующая посылка сообщения … Возврат из функции происходит сразу после инициализации передачи.
非阻塞消息发送…在传输初始化后立即从函数中返回
int MPI_Irecv … Неблокирующий приём сообщения. Возврат из функции происходит сразу после инициализации передачи.
非阻塞消息接收。函数返回在传输初始化后立即发生

int MPI_Iprobe(source, tag, comm, &flag, &status);
不会真正取走消息，只把即将到来的消息元数据填入 status，flag=1 表示已有匹配消息可收。

MPI_Isend   标准模式
MPI_Ibsend  带用户缓冲的缓冲模式
MPI_Issend  同步模式（要求接收方已开始接收才返回）
MPI_Irsend  就绪模式（发送前接收端必须已启动接收，最快但最不安全）

查询/等待完成的一组函数
阻塞等待：
MPI_Wait(request, &status)          — 等待单个
MPI_Waitall(n, requests[], statuses[]) — 等待全部
MPI_Waitany/ MPI_Waitsome           — 等待任意一个/至少一个
非阻塞探测：
MPI_Test(request, &flag, &status)   — 不等待，仅看是否完成
MPI_Testall/ Testany/ Testsome      — 对应 Wait 的测试版


## 37. MPI: коллективные операции.
В операциях коллективного взаимодействия процессов участвуют все процессы коммуникатора
在集体操作中，通信子中的所有进程都必须参与
...
● Упаковку данных
…
Производный тип данных характеризуется последовательностью базовых типов
и набором значений смещения относительно начала буфера обмена. Смещения могут
быть как положительными, так и отрицательными, не требуется их упорядоченность.»
为了传送异构数据，可以采用：
● 派生数据类型
● 数据打包
……
派生数据类型由一组基本类型和相对于通信缓冲区起始位置的偏移量序列来描述；偏移量可以是正数也可以是负数，且无需按顺序排列。

Для упаковки данных используется следующая функция
int MPI_Pack (…)
…
Упаковка incount элементов типа datatype из массива inbuf в массив outbuf
со сдвигом position байт. …
...
int MPI_Pack (…)
……
将 incount 个 datatype 类型的元素从数组 inbuf 打包到数组 outbuf，并按 position 字节偏移。……
已打包数据在发送时使用数据类型 MPI_PACKED。
……
相应地，解包数据时使用函数
int MPI_Unpack (…)

39. OpenMP: параллельная программа, нити, конструкции для организации параллельных и последовательных секций.  
OpenMP：并行程序、线程、并行与顺序段组织构造。

Любой процесс состоит из нескольких нитей управления (основного элемента
управления openMP), которые имеют общее адресное пространство, но разные потоки
команд и раздельные стеки. То есть идеология заключается в том, что нити рождаются
по необходимости, и когда эта необходимость пропадает, нити уничтожаются.
任何进程都包含多个控制线程（即openMP的核心管理单元），这些线程共享同
一地址空间，但具有独立的命令流和各自的堆栈。其核心理念在于：线程会根据实际
需求动态生成，当不再需要时则会被自动销毁。

openMP подразумевает только SPMD-модель (single Program Multiple Data)
параллельного программирования: для всех параллельных нитей используется один и
тот же код.
openMP仅支持SPMD-模型（single Program Multiple Data）并行编程：所有并行
线程使用相同的代码。

OpenMP-программа всегда состоит из последовательных и параллельных секций.
Последовательная секция: выполняет одна мастер-нить.
Параллельная секция: директива #pragma omp parallel порождает заданное число нитей,которые одновременно выполняют один и тот же код; при выходе — неявный барьер и остаётся только мастер-нить.
Секции могут чередоваться и вкладываться друг в друга, образуя общую структуру программы.
OpenMP 程序由串行区与并行区构成。
串行区：仅主线程运行。
并行区：#pragma omp parallel 生成指定数量线程，共同执行同段代码；退出时隐式同步，只留主线程。
两类区域可任意交替与嵌套，形成程序整体骨架


## 40. OpenMP: основные конструкции для распределения работы между нитями.
循环迭代拆分 #pragma omp for 里的子句
private(list)	每次迭代拥有私有副本，初值未定义。
firstprivate(list)	私有副本，并用串行值初始化。
lastprivate(list)	把最后完成的那次迭代的私有值写回主变量。
shared(list)	显式声明共享变量（默认已共享，写不写都行）。
reduction(op:list)	归约：+、*、-、&、|、^、&&、||、min、max…
schedule(kind[,chunk])	迭代划分策略：static / dynamic / guided / auto / runtime
collapse(n)	把紧嵌套 n 层循环合并成一个大迭代空间再分。
ordered	允许在循环体内再写 #pragma omp ordered 块，保证按串行顺序执行。
nowait	取消循环结束后的隐式路障。
通用任务拆分
if(cond)	parallel、task	决定是否“生成并行区/任务”，循环指令本身没有“生成”概念。
untied	task	任务调度专属。
depend(...)	task	任务依赖图专属。
copyprivate(list)	single	把 single 内结果广播出去，与循环无关。
num_threads(n)	parallel	决定线程组大小，循环指令只是“分活”，不建组。

## 40. OpenMP: 线程间工作分配主要构造
**循环迭代拆分** `#pragma omp for` 子句  
| 子句 | 作用 |
|---|---|
| `private(list)` | 每次迭代拥有私有副本，初值未定义 |
| `firstprivate(list)` | 私有副本，并用串行值初始化 |
| `lastprivate(list)` | 把最后完成的那次迭代的私有值写回主变量 |
| `shared(list)` | 显式声明共享变量 |
| `reduction(op:list)` | 归约：`+`、`*`、`-`、`&`、`|`、`^`、`&&`、`||`、`min`、`max`… |
| `schedule(kind[,chunk])` | 迭代划分策略：`static`/`dynamic`/`guided`/`auto`/`runtime` |
| `collapse(n)` | 把紧嵌套 n 层循环合并成一个大迭代空间再分 |
| `ordered` | 允许在循环体内再写 `#pragma omp ordered` 块，保证按串行顺序执行 |
...
| 子句 | 适用指令 | 作用 |
|---|---|---|
| `if(cond)` | `parallel`、`task` | 决定是否生成并行区/任务 |
| `untied` | `task` | 任务调度专属 |
| `depend(...)` | `task` | 任务依赖图专属 |
| `copyprivate(list)` | `single` | 把 single 内结果广播出去 |
| `num_threads(n)` | `parallel` | 决定线程组大小 |
```
