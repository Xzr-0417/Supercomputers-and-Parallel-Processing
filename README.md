...
--------------------------------------------------
一、什么叫“计算密集型任务”  
I. Что такое «вычислительно-ёмкие задачи»  
--------------------------------------------------
1. 定义  
   ‑ 主要瓶颈是“处理器运算速度”而非 I/O 或网络延迟  
   ‑ 运算量随问题规模 n 呈超线性/指数增长  
   ‑ 内存带宽相对充裕，CPU 常年 100 % 满载  

   Определение  
   ‑ Узкое место — скорость процессора, а не ввод-вывод  
   ‑ Объём вычислений растёт сверхлинейно или экспоненциально с ростом n  
   ‑ Пропускная способность памяти не является ограничением, CPU загружен полностью  

2. 评价指标  
   ‑ 计算复杂度 O(f(n)) — 浮点/整数运算总次数  
   ‑ 墙钟时间 T ≈ (总运算量) / (实用浮点性能)  
   ‑ 扩展性 — 节点数 p 增加时能否保持效率  

   Показатели оценки  
   ‑ Вычислительная сложность O(f(n)) — число арифметических операций  
   ‑ Календарное время T ≈ (общее число операций) / (практическая FLOPS)  
   ‑ Масштабируемость — сохраняется ли эффективность при росте числа узлов p  

--------------------------------------------------
二、中小学示例 – “两两最大公约数之和”  
II. Школьный пример – «сумма НОД всех пар»  
--------------------------------------------------
任务描述  
├─ 输入：整数 n ≤ 10⁵  
├─ 暴力法：枚举所有 C(n,2) 对，每对调用欧几里得算法  
└─ 复杂度：O(n² log n) ≈ 1.5×10⁸ 次除法（n=10⁴ 时）  

Описание задачи  
├─ Вход: целое n ≤ 10⁵  
├─ Метод: перебор всех C(n,2) пар, НОД каждой пары  
└─ Сложность: O(n² log n) ≈ 1.5×10⁸ делений при n=10⁴  

评估  
├─ 笔记本 20 GFLOPS → 约 7 秒  
├─ 若 n 扩大 10 倍，运算量增 100×log10≈330 倍 → 需 40 分钟  
└─ 明显受 CPU 速度限制，属计算密集型  

Оценка  
├─ Ноутбук 20 GFLOPS → ≈7 с  
├─ При n×10 объём вырастает в ~330 раз → 40 мин  
└─ Явно ограничено скоростью CPU, задача вычислительно-ёмкая  

--------------------------------------------------
三、科研示例 – “DNS 湍流直接数值模拟”  
III. Научный пример – «DNS турбулентности»  
--------------------------------------------------
背景（取自 IBM 701/704 历史）  
├─ 1953 年 IBM 701 首次实现“位并行运算”，让 32 位加法从 32 周期降到 1 周期  
├─ 1955 年 IBM 704 加浮点硬件，峰值性能 ≈ 几 kFLOPS  
└─ 如今 DNS 需要 10¹⁷ FLOPS，正是当年“位并行”思想延续 10¹² 倍放大  

Контекст (из истории IBM 701/704)  
├─ В 1953 г. IBM 701 внедрил разрядно-параллельную арифметику: 32-битное сложение за 1 такт  
├─ В 1955 г. IBM 704 добавил ускоритель с плавающей точкой, пик ≈ несколько кFLOPS  
└─ Современный DNS требует 10¹⁷ FLOPS — продолжение идеи «параллелизма» в 10¹² раз масштабнее  

任务规模  
├─ 网格 2048³ ≈ 8.6×10⁹ 节点  
├─ 每节点每步 1000 FLOP，共 10⁴ 步  
└─ 总计 8.6×10¹⁵ ×10³ = 10¹⁷ FLOP  

Расчётный объём  
├─ Сетка 2048³ ≈ 8.6×10⁹ ячеек  
├─ На ячейку 1000 FLOP, 10⁴ временных шагов  
└─ Итого 10¹⁷ операций  

评估  
├─ 1 PFLOPS 超算 → 10¹⁷ /10¹⁵ = 100 秒 ≈ 27 小时  
├─ 内存带宽需求 ≈ 每步 50 TB — 已可用 HBM2e/ccNUMA 分层内存（继承 IBM STRETCH“内存分体”思想）  
└─ 通信占比 <5 %，主要瓶颈仍是“裸”浮点能力 → 纯正计算密集型  

Оценка  
├─ Суперкомпьютер 1 PFLOPS → 10¹⁷/10¹⁵ = 100 с ≈ 27 ч  
├─ Потребная пропускная памяти ≈ 50 ТБ/шаг — решается HBM2e/ccNUMA (идея «расслоения памяти» из IBM STRETCH)  
└─ Доля коммуникаций <5 %, узкое место — сырой FLOPS → чисто вычислительно-ёмкая задача  

--------------------------------------------------
四、工业示例 – “整车碰撞显式有限元”  
IV. Промышленный пример – «краш-симуляция автомобиля»  
--------------------------------------------------
任务参数  
├─ 单元数 5×10⁶，显式时间步 1 µs，总物理时间 0.15 s → 150 000 步  
├─ 每单元每步 2000 FLOP（继承 IBM 704 浮点硬件传统，只是放大 10⁹ 倍）  
└─ 总运算量 5×10⁶ ×1.5×10⁵ ×2×10³ = 1.5×10¹⁵ FLOP  

Параметры задачи  
├─ 5×10⁶ элементов, шаг 1 мкс, 0.15 с → 150 000 шагов  
├─ На элемент 2000 FLOP/шаг (наследие аппаратного FPU IBM 704, масштаб ×10⁹)  
└─ Итого 1.5×10¹⁵ операций  

评估  
├─ 100 TFLOPS 集群 → 15 秒纯 CPU  
├─ 规模×2（单元×2，步长÷2）→ 运算量×8，通信面×4，仍远未碰 I/O 壁  
└─ 决定交期的要素是“浮点堆得够不够”，而非磁盘或网络 → 计算密集型  

Оценка  
├─ Кластер 100 TFLOPS → 15 с «чистого» CPU  
├─ Удвоение сетки и половинный шаг → ×8 вычислений, ×4 поверхности обмена, I/O всё ещё не предел  
└─ Фактор срока — «сколько FLOPS можно выжать», а не диск/сеть → вычислительно-ёмкая задача
...
--------------------------------------------------
一、 依赖关系：定义与核心概念  
I. Зависимость: определение и ключевые понятия  
--------------------------------------------------
- **基础定义**：两个操作（或语句）被称为**依赖的**，如果它们执行时**访问同一个变量（内存单元）**。这是比"信息联系"更广泛的关系，信息联系是依赖的一种特例。  
  ‑ **Базовое определение**: Две операции (оператора) называются **зависимыми**, если при их выполнении имеют место **обращения к одной и той же переменной (ячейке памяти)**. Это более широкое отношение, чем "информационная связь", которая является частным случаем зависимости.
- **实用优势**：确定依赖关系**比确定信息联系简单得多**，这促使了依赖图模型的发展。在这些模型中，通常只处理转换器类型的操作符。  
  ‑ **Практическое преимущество**: Устанавливать факт зависимости **значительно проще, чем факт информационной связи**, что привело к развитию графовых моделей зависимостей. В этих моделях обычно имеют дело только с операторами-преобразователями.
- **信息结构的基础性**：**信息结构**（基于依赖关系）是分析程序和算法属性的基础。  
  ‑ **Фундаментальность информационной структуры**: **Информационная структура** (основанная на зависимостях) — это основа анализа свойств программ и алгоритмов.

--------------------------------------------------
二、 两个关键概念及其意义  
II. Два ключевых понятия и их значение  
--------------------------------------------------
1.  **信息依赖作为等价转换的准则**  
    1.  **Информационная зависимость как критерий эквивалентности преобразований**  
    - **核心作用**：**信息依赖性决定了程序转换的等价性标准**。对程序进行任何转换（如优化、并行化）时，必须保持其信息依赖关系，才能确保程序语义（结果）不变。  
      ‑ **Ключевая роль**: **Информационная зависимость определяет критерий эквивалентности преобразований программ**. Любое преобразование программы (оптимизация, распараллеливание) должно сохранять её информационные зависимости для обеспечения эквивалентности семантики (результата).

2.  **信息独立作为并行性的资源**  
    2.  **Информационная независимость как ресурс параллелизма**  
    - **核心作用**：**信息独立性定义了程序和算法的并行性资源**。信息上独立的操作（即没有依赖关系的操作）**可以安全地并行执行**。因此，程序中信息独立部分的数量和结构决定了其潜在的并行程度。  
      ‑ **Ключевая роль**: **Информационная независимость определяет ресурс параллелизма программы и алгоритма**. Информационно независимые операции (не имеющие зависимостей) **могут выполняться параллельно без риска**. Таким образом, количество и структура независимых частей программы определяют её потенциальный параллелизм.

--------------------------------------------------
核心总结 / Ключевой вывод  
--------------------------------------------------
信息依赖与独立是程序分析与转换的一体两面：  
- **依赖关系**是**保持程序语义正确性**的约束（“什么不能变”）。  
- **独立关系**是**挖掘并行执行潜力**的机会（“什么可以同时做”）。  

Информационная зависимость и независимость — две стороны анализа и преобразования программ:  
- **Зависимость** — это ограничение для **сохранения корректности семантики** («что нельзя менять»).  
- **Независимость** — это возможность для **извлечения параллелизма** («что можно делать одновременно»).
...
--------------------------------------------------
一、 算法图的定义与性质  
I. Определение и свойства графа алгоритма  
--------------------------------------------------
1. **定义与构建 (Определение и построение)**  
   ‑ **算法图**是算法在固定输入数据下**信息依赖关系**的可视化表示，是一个**有向无环多重图**。  
   ‑ **顶点**：代表算法中的**操作**。通常与操作集合一一对应。  
   ‑ **弧 (边)**：从顶点 `u` 指向顶点 `v`，表示操作 `u` 的结果是操作 `v` 的一个**参数**（数据依赖）。没有依赖关系的操作之间没有弧。  
   ‑ 没有入边的顶点称为**输入顶点**，没有出边的顶点称为**输出顶点**。  

   Определение и построение  
   ‑ **Граф алгоритма** — это ориентированное представление **информационных зависимостей** алгоритма при фиксированных входных данных; это **ориентированный ациклический мультиграф**.  
   ‑ **Вершины**: Соответствуют **операциям** алгоритма. Обычно существует взаимно-однозначное соответствие.  
   ‑ **Дуги (рёбра)**: Дуга из вершины `u` в вершину `v` означает, что результат операции `u` является **аргументом** для операции `v` (зависимость по данным). Между независимыми операциями дуг нет.  
   ‑ Вершины без входящих дуг — **входные вершины**, без исходящих дуг — **выходные вершины**.  

2. **参数化与算法类型 (Параметризация и тип алгоритма)**  
   ‑ **参数化图**：算法图几乎总是**依赖于输入数据**（如数组大小），因此是一个参数化图——顶点和弧的数量及结构可能随参数变化。  
   ‑ **确定性算法**：程序**没有条件语句**。对于任何输入，程序中的**所有操作**与图的**所有顶点**存在一一对应关系。  
   ‑ **非确定性算法**：程序包含条件语句。对于不同的输入，只有程序的**一个子集的操作**与当前图的顶点一一对应。  

   Параметризация и тип алгоритма  
   ‑ **Параметризованный граф**: Граф алгоритма почти всегда **зависит от входных данных** (напр., размеров массивов) — это параметризованный граф, где число и структура вершин/дуг меняются.  
   ‑ **Детерминированный алгоритм**: В программе **нет условных операторов**. При любых входных данных существует взаимно-однозначное соответствие между **всеми операциями** программы и **всеми вершинами** графа.  
   ‑ **Недетерминированный алгоритм**: В программе есть условные операторы. При разных входных данных только **некоторое подмножество** операций программы соответствует вершинам графа.  

--------------------------------------------------
二、 关键路径及其意义  
II. Критический путь и его значение  
--------------------------------------------------
1. **关键路径的定义 (Определение критического пути)**  
   ‑ 关键路径是**有向无环图中长度最大的路径**（路径长度通常按弧的数量或操作的权重计算）。  
   ‑ 它代表了算法中**必须顺序执行的最长操作链**，决定了并行化后的**最小可能执行时间**的理论下界。  

   Определение критического пути  
   ‑ **Критический путь** — это **путь максимальной длины** в ориентированном ациклическом графе (длина обычно измеряется в количестве дуг или весе операций).  
   ‑ Он представляет **самую длинную цепочку последовательно зависимых операций** в алгоритме и определяет **теоретический нижний предел** минимального времени выполнения при параллелизации.  

2. **顶点标记与关键路径的关系 (Разметка вершин и связь с критическим путём)**  
   ‑ **Утверждение 4.1**: 任何具有 `n` 个顶点的有向无环图，都可以用 `s` 个索引（`s < n`）标记其顶点，使得对于任何从索引为 `i` 的顶点指向索引为 `j` 的顶点的弧，都满足 `i < j`。这是一种**拓扑排序**的体现。  
   ‑ **关键性质**：标记所需的最小不同索引数等于 **`L + 1`**，其中 `L` 是**关键路径的长度**。这直观地说明了至少需要 `L+1` 个时间步（层）才能执行完所有操作，且关键路径上的操作占据了这些连续的层。  
   ‑ 因此，分析关键路径的长度（作为问题参数的函数）是**算法并行化**和评估其潜在加速能力的关键任务。  

   Разметка вершин и связь с критическим путём  
   ‑ **Утверждение 4.1**: Любой ориентированный ациклический граф с `n` вершинами можно пометить индексами от 1 до `s` (`s < n`) так, что для любой дуги из вершины с индексом `i` в вершину с индексом `j` выполняется `i < j`. Это отражает **топологическую сортировку**.  
   ‑ **Ключевое свойство**: Минимальное число различных индексов, необходимых для разметки, равно **`L + 1`**, где `L` — **длина критического пути**. Это наглядно показывает, что для выполнения всех операций требуется как минимум `L+1` временных шагов (уровней), причём операции на критическом пути занимают эти последовательные уровни.  
   ‑ Таким образом, нахождение длины критического пути как функции параметров задачи — ключевая задача при **распараллеливании алгоритмов** и оценке потенциального ускорения.  

--------------------------------------------------
核心结论 / Ключевой вывод  
--------------------------------------------------
算法图是分析计算依赖性和并行性的基础模型。**关键路径的长度直接决定了算法固有的顺序瓶颈，是评估并行算法最大可能加速比和指导优化（如负载平衡）的根本依据。**  

Граф алгоритма — это фундаментальная модель для анализа вычислительных зависимостей и параллелизма. **Длина критического пути непосредственно определяет присущее алгоритму последовательное «узкое место» и является основой для оценки максимально возможного ускорения параллельного алгоритма и руководства оптимизациями (например, балансировкой нагрузки).**
