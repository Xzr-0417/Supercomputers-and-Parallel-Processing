...
Вместе с сообщением по сети также посылается некий набор атрибутов: номер процессаотправителя, номер процесса-получателя, идентификатор сообщения, коммуниктор.  
与网络消息一同发送的还包含一组属性：发送方进程号、接收方进程号、消息标识符、通信者。

Процессы также объединяются в группы – упорядоченные множества процессов.  
От понятия группы нужно отличать понятие коммуникатора – контекста обмена группы. Это глобальный объект. Он объединяет различные процессы.  
流程还被整合为组——即有序的流程集合。  
需要区分“群组”概念与通信器——即群组交换的上下文环境。通信器是全局性对象，它整合了各类进程。
...
| 函数 | 描述 |
|------|------|
| `MPI_Ssend` | 传递消息并同步：只有当接收进程已开始接收时才解除阻塞。 |
| `MPI_Recv` | 阻塞接收：直到所需消息完全到达并拷入用户缓冲区才返回。 |

### 各类阻塞发送
| 模式 | 行为 | 返回条件 |
|---|---|---|
| `MPI_Send` | 标准阻塞发送 | 只保证“消息已送出缓冲区”，是否到达/被接收不保证 |
| `MPI_Bsend` | 带用户缓冲的阻塞发送 | 需先注册缓冲，数据拷进缓冲即可返回 |
| `MPI_Ssend` | 真正同步发送 | 只有当对方已开始 `MPI_Recv` 才返回，最严格同步 |
| `MPI_Rsend` | 就绪发送 | 调用时对方必须已启动接收，否则行为未定义（最快但最不安全） |

### 死锁（тупиковые ситуации）
**经典死锁例子**  
进程 0：`MPI_Send(→1, …) → MPI_Recv(←1, …)`  
进程 1：`MPI_Send(→0, …) → MPI_Recv(←0, …)`  
双方都在阻塞 `Send` 里等缓冲区，又都无法到达 `Recv`，永远互相等待。

**“偶然成功”的伪解**  
把顺序颠倒：  
进程 0：`MPI_Recv(←1, …) → MPI_Send(→1, …)`  
进程 1：`MPI_Recv(←0, …) → MPI_Send(→0, …)`  
如果系统内部缓冲足够，程序能跑通；但一旦缓冲不足就会再次死锁——不安全。

**三种正规解法**  
1. 交叉收发：  
   进程 0：`Send(→1) → Recv(←1)`  
   进程 1：`Recv(←0) → Send(→0)`  
2. 使用非阻塞操作（`Isend`/`Irecv` + `Wait`/`Test`）  
3. 直接用 `MPI_Sendrecv` 或 `MPI_Sendrecv_replace`——一次调用完成“收发对”，系统内部保证无死锁。

---

## 36. MPI: асинхронное взаимодействие процессов.
- 避免死锁（deadlock）  
- 提高利用率：消息传输同时可继续计算，不必空等。

| 非阻塞函数 | 描述 |
|------------|------|
| `MPI_Isend` | 非阻塞发送，传输初始化后立即返回 |
| `MPI_Irecv` | 非阻塞接收，传输初始化后立即返回 |
...
| 函数 | 描述 |
|---|---|
| `MPI_Wait` / `MPI_Waitall` / `MPI_Waitany` / `MPI_Waitsome` | 阻塞等待 |
| `MPI_Test` / `MPI_Testall` / `MPI_Testany` / `MPI_Testsome` | 非阻塞探测 |

---

## 37. MPI: коллективные операции.
- 所有进程必须参与  
- 不使用消息标签（tag），操作顺序由程序中出现顺序决定  
...
派生数据类型由一组基本类型和相对于通信缓冲区起始位置的偏移量序列描述；偏移量可正可负，无需排序。

**打包/解包函数**  
```c
int MPI_Pack(...)   // 将数据打包到 outbuf
int MPI_Unpack(...) // 从缓冲区解包
```
已打包数据使用类型 `MPI_PACKED`。

---

## 39. OpenMP: 并行程序、线程、并行与顺序段组织构造
任何进程包含多个控制线程（OpenMP 核心管理单元），共享地址空间，各自独立命令流与堆栈。  
线程按需动态生成与销毁。

OpenMP 仅支持 SPMD 模型：所有并行线程使用相同代码。
